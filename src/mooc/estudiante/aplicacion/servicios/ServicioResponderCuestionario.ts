...
export class ServicioResponderCuestionario implements IServicioAplicacion {
    ...
    public async ejecutar (comando: ResponderCuestionarioComandoDTO): Promise<Resultado<any>> {

        try{
            //Verificamos curso y estudiante
            const estudiantePersistencia = await this.repositorioEstudiantes.consultar(comando.uuidEstudiante)
            ...
            const cursoPersistencia = await this.repositorioCursos.consultar({uuid: comando.uuidCurso})
            ...
            const consultarPreguntas = await this.repositorioRespuestas.consultar({uuidCuestionario: comando.uuidCuestionario})
    
            //Mapeamos a dominio
            const preguntasDominio = PreguntaMapeador.ConvertirRespuestasDominio(consultarPreguntas)

            //Verificamos si aprueba
            const verificar = VerificarCuestionarioAprobado.verificar(preguntasDominio, comando.respuestasCuestionario)

            if(verificar)
                estudianteDominio.cursoAprobado(cursoDominio.uuid)
            else
                estudianteDominio.cursoReprobado(cursoDominio.uuid)
