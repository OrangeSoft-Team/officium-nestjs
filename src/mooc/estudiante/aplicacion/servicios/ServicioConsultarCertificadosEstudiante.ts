...
import {
  ConsultarCertificadoEstudianteQueryDTO,
  ConsultarCertificadoEstudianteRespuestaDTO,
} from '../dto/queries/ConsultarCertificadosEstudiante.query'
...
export class ServicioConsultarCertificadosEstudiante
  implements IServicioAplicacion
{
  ...
  public async ejecutar(
    query: ConsultarCertificadoEstudianteQueryDTO,
  ): Promise<Resultado<any>> {
    try {
      //Busca si el estudiante existe
      const estudianteExiste = await this.repositorioEstudiantes.existe(
        query.uuidEstudiante,
      )
      if (!estudianteExiste)
      ...
      //Mapea a dominio
      const certificadosDominio =
        CertificadoMapeador.ConvertirListaCertificadosDominio(certificados)

      //Mapea respuesta
      const respuesta =
        CertificadoMapeador.ConvertirListaCertificadosRespuesta(
          certificadosDominio,
        )
      return Resultado.ok<ConsultarCertificadoEstudianteRespuestaDTO[]>(
        respuesta,
      )
    } catch (error) {
