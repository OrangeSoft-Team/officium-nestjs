...
export class ServicioConsultarListaCursos implements IServicioAplicacion{
    ...
    public async ejecutar(): Promise<Resultado<any>> {
        try{
            ...
            const listaCursosDominio = CursoMapeador.ConvertirListaCursosDominio(listaCursosPersistencia)
            const respuesta = CursoMapeador.ConvertirListaCursosRespuesta(listaCursosDominio)
            return Resultado.ok<ConsultarListaCursosRespuestaDTO[]>(
                respuesta,
              )
        }
        catch(error){
