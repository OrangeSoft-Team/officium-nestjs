...
import { EstudianteInactivo, EstudianteNoExiste } from "../../dominio/excepciones/estudiante/Estudiante.excepciones";
...
export class ServicioInscribirCursoEstudiante implements IServicioAplicacion {
    ...
    public async ejecutar(comando: InscribirseCursoComandoDTO): Promise<Resultado<any>>{
        try{
            ...
            if (ConsultarEstatusEstudiante.consultar(estudiantePersistencia.estatus))
                throw new EstudianteInactivo('El estudiante no puede ser inscrito, se encuentra suspendido')
