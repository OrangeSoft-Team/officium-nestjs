...
import { DatosRestaurarOpcionesCorrecto, DatosRestaurarRespuestas } from "./RestaurarPreguntas";
    ...
    public static verificar(
        ...
        preguntas.forEach((pregunta)=>{
            ...
            let opcion = pregunta.opciones.find((res) => {
               return res.uuid.esIgual(IdentificadorOpcion.crear(
                  respuestas.find(obj => {
            ...
            if(opcion){
                if(opcion.correcto.obtenerCorrecto()){
                    valorAcumulado = valorAcumulado + pregunta.ponderacion.obtenerPonderacion()
                }
            }
        })
