  ...
  public async ejecutar() {
    try {
      ...
      let ofertasLaborales = lista.map((listadoOfertas) =>
        OfertaLaboralMapeador.MapConsultaDominioOferta(listadoOfertas),
      )
      ...
      const dtoConsulta = ofertasLaborales.map((listadoOfertas) =>
        OfertaLaboralMapeador.MapConsultaRespuestaOferta(listadoOfertas),
      )

      //Se retorna el DTO
      return Resultado.ok<ConsultarOfertasLaboralesDTO[]>(dtoConsulta)
    } catch (error) {
